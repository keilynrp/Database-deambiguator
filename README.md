...
![Tests](https://img.shields.io/badge/Tests-4036_collected-28A745?logo=pytest&logoColor=white)
...
<!-- BEGIN GENERATED REPOSITORY METRICS: tree-backend-tests -->
  tests/                 313 test files, 4036 tests
...
| Testing | pytest (4036 tests collected), Vitest (333 tests), Playwright |
...
```bash
# Run all backend tests (exhaustive — same suite the "test-shard" CI jobs union to)
pytest backend/tests -q

# With coverage
pytest backend/tests --tb=short --cov=backend --cov-report=term-missing -q

# Focused marker run — taxonomy: unit, contract, integration, reporting,
# security, postgres, slow (registered + enforced in pyproject.toml)
pytest backend/tests -m security -q
...
**Current test stats:** 4036 backend tests collected across 313 test files. Frontend: 333 Vitest tests.
