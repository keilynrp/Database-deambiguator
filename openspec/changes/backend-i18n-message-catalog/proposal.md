...
Meanwhile the frontend already has the standard — `frontend/app/i18n/translations.ts` with ~3,400 keys and a CI gate requiring EN+ES parity for every new one. The system has a documented convention that the backend does not participate in. Translating the offending strings in place would just make them English instead of Spanish, still with no way to serve a Spanish-speaking reader, and still with nothing stopping the next Spanish literal from being committed.
...
- Extend the **EN/ES parity gate** to cover the backend catalog, so a backend string cannot ship in one language only.
...
- **CI**: the existing translation parity gate is extended to the backend catalog.
