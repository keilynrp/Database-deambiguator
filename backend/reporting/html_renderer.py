...
def render_html_document(doc: LocalizedReportDocument) -> str:
    ...
    # every numbered section, matching that same original order.
    stakeholder = next((s for s in doc.sections if s.key == "stakeholder_reading"), None)
    numbered = [s for s in doc.sections if s.key != "stakeholder_reading"]
    ...
    body_sections.extend(render_html(section) for section in numbered)
    # A collector that raised no longer keeps its original position among its
    # siblings (#292): errors were previously interleaved in request order
    # with successful sections, and now render after every successful one.
    # Disclosed deviation — this path is a per-section failure, not the
    # common case, and reordering it costs nothing but a tidier aggregate.
    for error in doc.errors:
        body_sections.append(
            f'<section><h2>{error.title}</h2>'
            f'<p style="color:#ef4444">Error building section: {error.detail}</p></section>'
        )
    ...
    footer = f'<footer>Generated by UKIP &nbsp;·&nbsp; {doc.generated_at}</footer>'
