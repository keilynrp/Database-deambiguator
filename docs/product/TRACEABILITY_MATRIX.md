...
| Metricas cienciometricas de revista (open proxy) | EPIC-004 | US-082 | SPRINT-105 | `backend/analyzers/journal_normalization.py`, `backend/analyzers/journal_normalization_bayes.py`, `backend/routers/journals.py`, `backend/scripts/backfill_nif_bayes.py`, `frontend/app/analytics/journals/`, `docs/SCIENTOMETRICS.md`, `docs/product/stories/US-082-journal-scientometric-metrics.md` | NIF + NIF Bayes (IC 95%), APC, DOAJ, works en dashboard de revistas, modal y ficha de detalle | Done |
| Clasificacion y facet de tipo de obra | EPIC-004 | US-083 | SPRINT-105 | `backend/services/work_type.py`, `backend/adapters/enrichment/openalex.py`, `backend/services/entity_service.py`, `backend/scripts/backfill_work_type.py`, `frontend/app/lib/workType.ts`, `frontend/app/components/FacetPanel.tsx`, `docs/product/stories/US-083-work-type-facet.md` | Facet `work_type` + filtro `ft_work_type`, badges en tabla/modal/ficha | Done |
...
| Gobierno de controles | EPIC-018 | ER-CTRL-001 | Unscheduled | `ENTERPRISE_READINESS_PROGRAM.md`, `ENTERPRISE_CONTROL_REGISTER.md`, `docs/product/evidence/RC-2026-08-26-01.md` | Primer ciclo de evidencia de release ejecutado (RC-2026-08-26-01); reconciliacion P0/P1 completa, atestacion de owner pendiente | specified |
...
| Recovery medido | EPIC-018 | US-073 / ER-BCP-001 | Unscheduled | `backend/backup_assurance.py`, `backend/scripts/validate_restore.py`, `docker-compose.prod.yml`, `docs/operating/BACKUP_RESTORE_RUNBOOK.md` | Repository controls and runbook implemented; provider configuration, two successful backup cycles, and the first isolated restore drill remain pending. | specified |
