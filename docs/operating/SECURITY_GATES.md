...
| Generic BYOK credential fields (`StoreConnection.api_key/api_secret/access_token`, `AIIntegration.api_key`) | `backend/models.py`, `backend/schemas.py`, `backend/routers/stores.py`, `backend/routers/ai_rag.py` | user-supplied opaque string (GitHub is not a supported `StoreConnection` platform today) | opaque pass-through, encrypted at rest (Fernet) | none — Pydantic fields carry no `max_length`; DB columns are unbounded `sa.String`/Postgres `VARCHAR` | none — already accepts arbitrary-length opaque values | Regression tests added: `backend/tests/test_github_token_opaque_compat.py` proves a 520-char synthetic opaque token round-trips through encrypt/decrypt and the `/stores` and `/ai-integrations` endpoints with no truncation or rejection. |
