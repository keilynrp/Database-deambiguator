...
- baseline recomendado: crear un `AlertChannel` activo suscrito a `ops.check_failed` y ejecutar `POST /ops/checks/run?notify=true` desde cron, monitor externo o job de plataforma.
