...
Los cuatro niveles del issue #296, hechos operacionalmente verificables. La asignación de cada capacidad en §3 se basa en evidencia del repositorio (gates de CI, cobertura de tests, documentación de ADR/EPIC, uso real) — nunca en intuición.
...
| Protección de test/release | Cubierta por al menos **un gate de CI bloqueante dedicado**, más allá de la suite general — p. ej. el gate F1 de resolución de entidades, la corrida exhaustiva contra PostgreSQL, el smoke de SDK con auth+scope real, `frontend-e2e-critical`, los gates de drift de OpenAPI/SDK. |
| Obligación de documentación | Debe tener mapa de frontera explícito (§4), fila de release-gate (§6), y reflejarse en los hechos generados del README (#295) cuando aplique. |
| Expectativa de incidente/soporte | Incidentes de producción que la afectan son severidad 1/2 por defecto; los controles P0/P1 de `ENTERPRISE_CONTROL_REGISTER.md` que la tocan aplican. |
| Qué NO califica | Capacidades sin protección de CI dedicada más allá de la suite general; capacidades sin las cuales el producto sigue funcionando (opcionales/con fallback); herramientas admin internas; cualquier cosa documentada explícitamente como legacy. |
...
| Protección de test/release | Cubierta por la suite exhaustiva general de backend (shards + partition-union guard, #293) y/o la suite Vitest de frontend; no tiene un gate dedicado propio. |
...
| Qué NO califica | Capacidades detrás de un flag apagado por defecto sin ventana de observación en producción (eso es Tier 3); herramientas admin-only sin contrato de cliente (Tier 3); cualquier cosa cuyo control de gobierno asociado siga por debajo de `implemented` en el registro de enterprise-readiness. |
...
| Protección de test/release | Cubierta solo por la suite general donde exista; puede tener gate de CI dedicado más angosto o inexistente. |
...
| Qué NO califica | Cualquier cosa protegida por un gate de calidad bloqueante dedicado (eso es evidencia de Tier 1); cualquier cosa de la que dependa el loop ingesta→canónico→autoridad→enriquecimiento para funcionar. |
...
| **Cola de jobs durable en background (ADR-007)** | 2 | FSM de jobs, claim/lease/retry, worker, scheduler, migración incremental fuera del proceso web | `backend/jobs/` (7 módulos), `backend/routers/jobs.py` | PostgreSQL (lease queue, broker-free) | REST `/jobs/*`; ADR-007 | ADR + runbook + topología + checklist de cutover propios; migración por dominio vía flags `UKIP_JOBS_<DOMAIN>` |
...
Para cada capacidad Tier 1/2: qué pertenece dentro, qué debe quedar fuera, dirección de dependencia permitida, contrato usado a través de la frontera, y dónde se tolera acoplamiento cruzado hoy (y por qué).
...
| Cola de jobs durable (ADR-007) | FSM de jobs, claim/lease/retry, handlers registrados | Lógica de negocio de cada dominio (delega a `enrichment_worker`, `scheduled_imports`, etc.) | Otras capacidades encolan trabajo; el runtime no conoce el dominio de negocio más allá del handler registrado | `runtime.register_handler(name, fn)`; ADR-007 | Migración por dominio vía flags `UKIP_JOBS_<DOMAIN>` — algunos dominios (report, import, enrichment) migrados; otros (staleness scheduler) aún no, documentado como incremental |
...
**Sin gate de CI dedicado propio hoy:** Agentic/RAG/NLQ, Integración comercial legacy, Herramientas admin, Demo/Ventas — consistente con su asignación Tier 3/4.
...
**Dirección propuesta:** ninguna acción en este PR; queda como el próximo paso documentado de la migración ADR-007 ya en curso.
