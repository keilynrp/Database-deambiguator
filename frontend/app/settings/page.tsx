...
function FaviconDropZone({
    ...
    const upload = useCallback(async (file: File) => {
        ...
        if (!allowed.includes(file.type)) {
            setError("Accepted formats: ICO, PNG, SVG.");
            return;
        }
        if (file.size > FAVICON_MAX_KB * 1024) {
            setError(`File is too large. Maximum is ${FAVICON_MAX_KB} KB.`);
            return;
        }
        setUploading(true);
        try {
            const fd = new FormData();
            fd.append("file", file);
            const res = await apiFetch("/branding/favicon", { method: "POST", body: fd });
            if (!res.ok) {
                const err = await res.json().catch(() => ({ detail: "Upload failed" }));
                throw new Error(err.detail ?? "Upload failed");
            }
            const data = await res.json();
            onUploaded(data.favicon_url);
        } catch (e: any) {
            setError(e.message ?? "Upload failed");
        } finally {
            setUploading(false);
    ...
    return (
        <div>
            <label className="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">
                Favicon
                <span className="ml-1 text-gray-400 font-normal">(ICO, PNG, SVG · max {FAVICON_MAX_KB} KB)</span>
            </label>
            <div
                onDragOver={e => { e.preventDefault(); setDragging(true); }}
                onDragLeave={() => setDragging(false)}
