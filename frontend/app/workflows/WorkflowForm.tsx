...
function ConditionEditor({
  ...
  return (
    <div className="flex items-start gap-2 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/70">
      <div className="grid flex-1 grid-cols-3 gap-2">
        <select
          value={condition.type}
          onChange={(e) => onChange(index, { ...condition, type: e.target.value as ConditionType })}
          className="rounded border border-slate-300 bg-white px-2 py-1.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
        >
        ...
        <input
          ...
          className="rounded border border-slate-300 px-2 py-1.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500"
      ...
      <button
        ...
        className="mt-1 text-slate-400 hover:text-rose-500 dark:text-slate-500 dark:hover:text-rose-400"
    ...
    <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/70">
      <div className="flex items-center gap-2">
        <select
          ...
          className="flex-1 rounded border border-slate-300 bg-white px-2 py-1.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
        >
      ...
      {action.type === "send_webhook" && (
        <input
          ...
          className="w-full rounded border border-slate-300 px-2 py-1.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500"
        />
      )}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500"
          />
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-200">Trigger</label>
          <select
            value={trigger}
      ...
      <div>
        <div className="mb-2 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200">
            Conditions{" "}
            <span className="font-normal text-slate-400 dark:text-slate-500">(all must match - leave empty to always run)</span>
          </h3>
          <button
            type="button"
        ...
        <div className="space-y-2">
          {conditions.length === 0 && (
            <p className="py-2 text-sm italic text-slate-400 dark:text-slate-500">No conditions - workflow always runs.</p>
          )}
