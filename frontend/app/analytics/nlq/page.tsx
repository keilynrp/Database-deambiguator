...
const EXAMPLE_KEYS = [
  "page.nlq.example.publications_by_year",
  "page.nlq.example.countries",
  "page.nlq.example.fields",
  "page.nlq.example.enrichment_status",
  "page.nlq.example.publications_2022",
  "page.nlq.example.source_distribution",
];
...
export default function NLQPage() {
  ...
  const ask = useCallback(async (q: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.detail ?? t("page.nlq.query_failed"));
      } else {
        setResult(data);
        Analytics.olapQuery(activeDomainId, data.translated.group_by.length);
      }
    } catch {
      setError(t("page.nlq.network_error"));
    } finally {
      setLoading(false);
    }
  }, [activeDomainId, t]);
  ...
  return (
    <div className="flex flex-col gap-8 pb-12">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
        ...
        <span>/</span>
        <span className="text-gray-900 dark:text-white font-medium">{t("nav.nlq")}</span>
        {activeDomain && (
      ...
      <form onSubmit={handleSubmit} className="mx-auto w-full max-w-2xl">
        <div className="relative">
          ...
          <input
            ...
            placeholder={t("page.nlq.placeholder")}
            disabled={loading}
          ...
          >
            {loading ? t("page.nlq.thinking") : t("page.nlq.ask_ai")}
          </button>
        </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-3.5 dark:border-gray-800">
              <div className="flex items-center gap-3">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white">{t("common.results")}</h3>
                <span className="rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-400">
                  {t("page.nlq.results_summary", { groups: result.result.rows.length, records: result.result.total.toLocaleString() })}
                </span>
                {isCrossTab && (
                  <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                    {t("page.nlq.cross_tab")}
                  </span>
                )}
              </div>
              <Link
                        ...
                        >
                          <td className="px-5 py-3 font-medium text-gray-900 dark:text-white">
                            {row.values[result.result.group_by[0]] ?? (
                              <span className="italic text-gray-400">{t("page.olap.null_value")}</span>
                            )}
                          </td>
                          {isCrossTab && (
                ...
                {hasMore && (
                  <div className="flex items-center justify-center gap-3 border-t border-gray-100 py-3 dark:border-gray-800">
                    <span className="text-xs text-gray-400">
                      {t("page.nlq.more_rows", { count: result.result.rows.length - visibleRows })}
                    </span>
                    <button
      ...
      {!result && !loading && !error && (
        <div className="mx-auto flex w-full max-w-lg flex-col items-center gap-3 py-6 text-center">
          <div className="grid grid-cols-3 gap-3 opacity-60">
            {[
              t("page.nlq.empty_chip_year"),
              t("page.nlq.empty_chip_country"),
              t("page.nlq.empty_chip_fields"),
            ].map((item) => (
              <div key={item} className="rounded-xl border border-gray-200 bg-white p-3 text-xs text-gray-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400">
                {item}
              </div>
            ))}
          </div>
          <p className="mt-2 text-xs text-gray-400 dark:text-gray-500">
            {t("page.nlq.ai_provider_required_prefix")}{" "}
            <Link href="/integrations" className="text-violet-600 hover:underline dark:text-violet-400">
              {t("page.nlq.ai_provider_required_link")}
            </Link>
          </p>
        </div>
      )}
